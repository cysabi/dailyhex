...
func (s Styles) New(r *lipgloss.Renderer, secret string) Styles {
	return Styles{
		...
		Base:         r.NewStyle(),
		Title:        r.NewStyle().Width(mainWidth).AlignHorizontal(lipgloss.Center),
		...
		ColorBox:     r.NewStyle().Width(2).Height(1),
		InputBox:     r.NewStyle().Border(lipgloss.RoundedBorder()).PaddingLeft(1),
		MoveBox:      r.NewStyle().Padding(1, 2, 0),
